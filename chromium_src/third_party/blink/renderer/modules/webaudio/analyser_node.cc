...
void AnalyserHandler::SetFudgeFactor(double fudge_factor) {
  analyser_.fudge_factor_ = fudge_factor;
}

}  // namespace blink

#define BRAVE_ANALYSERNODE_CONSTRUCTOR \
  GetAnalyserHandler().SetFudgeFactor( \
      brave::GetFudgeFactor(To<Document>(GetExecutionContext())));
